...
$recupChamp = "SELECT * FROM utilisateur";
...
if (count($contrat) <> 0) {
    $k = 0;
  for ($i=0; $i < count($recupChamp); $i++) {
    for ($j=0; $j <count($contrat) ; $j++) {
      if ($recupChamp[$i]->refUti <> $contrat[$j]->refUti) {
        $pasDeContrat[$k] = $recupChamp[$i];
        $k++;
      }
    }
  }
}else{
  $pasDeContrat = $recupChamp;
}
...
if(isset($_POST['submit'])){
   ...
   if(empty($_POST['refUti']) or  empty($_POST['reftypCont']) or empty($_POST['datedebutCont']) or empty($_POST['datefinCont']) or empty($_POST['descriptypCont'])){
             danger('Tous les champs doivent être completés !');

    }else{
     $addUser = "INSERT INTO contrat(refUti, reftypCont, datedebutCont, datefinCont, descripCont) VALUES (?,?,?,?,?)";
            ...
            $_POST['datedebutCont'],
            $_POST['datefinCont'],
            $_POST['descriptypCont']

            ]);

            info('Contrat du personnel geré');
    }
...
<head>
  ...
  <title>Gerer les contrats</title>
    ...
    <div id="content-wrapper" class="d-flex flex-column">
      ...
      <div id="content">
        ...
        <div class="container-fluid">
            ...
            <h1 class="animated bounceInDown text-primary">GERER LES CONTRATS</h1>
            <hr>
            <?= getMessages(); ?>
          ...
          <div class="row">
              ...
              <div class="form-group col-md-4 mb-4">
                <label for="refUti">Nom du personnel<span> *</span> : </label>
                  <select id="refUti"  name="refUti" required class="form-control form-control-md">
                    <option value=""></option>
                      <?php foreach($pasDeContrat as $champ): ?>
                    <option value="<?=$champ->refUti; ?>"><?=$champ->nomUti; ?> <?=$champ->pnomUti; ?></option>
                      <?php endforeach; ?>
                  </select>
              </div>
            ...
            <div class="row">
              ...
              <div class="form-group col-md-4 mb-4">
                <label for="reftypCont">Type de contrat<span> *</span> : </label>
                  <select id="reftypCont"  name="reftypCont" required class="form-control form-control-md">
                    <option value=""></option>
            ...
            <div class="row">
              ...
              <div class="form-group col-md-4 mb-4">
                 <label for="datedebutCont">Date de debut<span> *</span> : </label>&nbsp;
                 <input type="date" id="datedebutCont"  name="datedebutCont" required  class="form-control form-control-md">
              </div>
              <div class="form-group col-md-4 mb-4">
              </div>
            </div>
            ...
            <div class="row">
              ...
              <div class="form-group col-md-4 mb-4">
                  <label for="datefinCont">Date de fin<span> *</span> : </label>&nbsp;
                  <input type="date" name="datefinCont"  id="datefinCont"  required class=" form-control form-control-md">
              </div>
              <div class="form-group col-md-4 mb-4">
              </div>
            </div>
            ...
            <div class="row">
              ...
              <div class="form-group col-md-4 mb-4">
                  <label for="descripCont">Description<span> *</span> : </label>&nbsp;
                  <textarea type="textarea" name="descriptypCont" rows="5" id="descripCont"  required class="form-control form-control-md"></textarea>
              </div>
              <div class="form-group col-md-4 mb-4">
              </div>
            </div>
